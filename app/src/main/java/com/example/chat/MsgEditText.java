...
public class MsgEditText extends AppCompatEditText {
    ...
    private MsgEditTextListener msgEditTextListener;
    ...
    public void setMsgEditTextListener(MsgEditTextListener msgEditTextListener) {
        this.msgEditTextListener = msgEditTextListener;
    }

    public void clearCustIds() {
        custIdList.clear();
    }
    ...
    public void addAtSpan(String maskText, String showText, String userId) {
        if (custIdList != null && custIdList.contains(userId)) {
            return;
        }
        custIdList.add(userId);
        builder = new StringBuilder();
        if (!TextUtils.isEmpty(maskText)) {            //已经添加了@
        ...
        if (lengthBefore == 1 && lengthAfter == 0) {
            ...
            for (MyTextSpan myImageSpan : spans) {
                if (getText().getSpanEnd(myImageSpan) == start && !text.toString().endsWith(myImageSpan.getShowText())) {
                    ...
                    if (custIdList != null && custIdList.contains(myImageSpan.getUserId())) {
                        custIdList.remove(myImageSpan.getUserId());
                    }
                    break;
                }
            }
        }
        if (msgEditTextListener != null) {
            msgEditTextListener.onTextChanged(text);
        }
    }
    ...
    public interface MsgEditTextListener {
